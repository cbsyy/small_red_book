...
import { RiAddLine, RiDeleteBinLine, RiArrowUpLine, RiArrowDownLine, RiLoader4Line, RiCheckLine, RiArrowLeftLine, RiRefreshLine, RiRobot2Line, RiAlertLine } from 'react-icons/ri';
...
export default function OutlineStep() {
  ...
  const [usedPromptConfig, setUsedPromptConfig] = useState<string>('');
  ...
  useEffect(() => {
    const handleRetry = () => {
      if (!isRegenerating && !isGeneratingOutline) {
        handleGenerateOutline();
      }
    };
    window.addEventListener('retry-outline', handleRetry);
    return () => window.removeEventListener('retry-outline', handleRetry);
  }, [isRegenerating, isGeneratingOutline, originalText, selectedProfileId]);
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      // 记录使用的 Prompt 配置
      if (data.data?.promptConfig) {
        setUsedPromptConfig(data.data.promptConfig);
      }

      // 转换为带 id 的格式（支持新的字段结构）
      const newOutline = (data.data?.cards || data.data?.outline || data.outline || []).map((item: any, index: number) => ({
        ...
        imagePromptAutoGenerated: item.imagePromptAutoGenerated || false,
  ...
  const handleProceedToPromptEdit = async () => {
    if (outline.length === 0) {
      setError('请至少添加一个大纲项');
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl text-blue-700 text-sm">
        ...
        {usedPromptConfig && (
          <span className="ml-2 text-blue-500">（当前使用: {usedPromptConfig}）</span>
        )}
      </div>
        ...
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-amber-700 font-medium text-sm mb-1">自动补全提示</p>
              {warnings.map((warning, index) => (
                <p key={index} className="text-amber-600 text-sm">{warning}</p>
              ))}
              <p className="text-amber-500 text-xs mt-2">
                系统已根据卡片内容自动生成图片提示词，你可以在下一步手动修改
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-4">
        {outline.length === 0 ? (
          <div className="p-8 bg-gray-50 border border-gray-200 rounded-xl text-center">
            <p className="text-gray-500 mb-2">暂无大纲内容</p>
            <p className="text-gray-400 text-sm">
              点击上方「生成大纲」按钮，或手动添加大纲项
            </p>
          </div>
        ) : (
      ...
      <div className="flex justify-between items-center pt-4 border-t border-gray-100">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={handleProceedToPromptEdit}
